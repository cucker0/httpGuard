...
if ( hgOn and not (hgModule == "off") ) or hgModule == "on" then --判断是否应用HttpGuard
	
	
	if not Guard:ipInByDenyList(ip) then		-- not in byDeny名单
		--随机延时处理URL
		...
		if not Guard:ipInByWhiteList(ip) then		-- not in 白名单
		...
		if not Guard:urlFilterAllow(userAgent,httpReferer,ip,reqUri,address) then	--除了允许所有IP访问的URL外,其他执行拒绝操作.
